...
while total_calls < 1000:  # Example total calls target
    if next_call_arrival <= current_time:
        ...
        if active_calls < queue_size:
            call_end_time = current_time + call_duration
            gp_queue.append((call_end_time, call_type))
            active_calls += 1
            accepted_calls += 1
        else:
            dropped_calls += 1
        ...
        if gp_queue:
            gp_queue.sort()
            next_call_end_time, call_type = gp_queue.pop(0)
            current_time = next_call_end_time
            active_calls -= 1
            if call_type == 'area_specific':
                if area_specific_active_calls < 1:
                    ...
                    gp_queue.append((current_time, 'area_specific'))
                    active_calls += 1
